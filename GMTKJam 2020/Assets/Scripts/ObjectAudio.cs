...
public class ObjectAudio : MonoBehaviour {
    ...
    public void Init () { // This is called from the gridobject script, after the data has been loaded into it (can be used sans data, in other words)
        //soundDict.Clear ();
    ...
    public void PlayRandomSoundType (SoundType type) { // play one random sound
    ...
    public void PlayAllSoundsInOrder (SoundType type) { // play all sounds in the order they are in the list
        ObjectSound soundObj = null;
        soundDict.TryGetValue (type, out soundObj);
        randomSounds.Clear ();
        if (soundObj != null) {
